...
exports.createUser = function createUser(req, res, next) {
  ...
  workflow.on('validateUser', function validateUser() {
    ...
    req.checkBody('user_type', 'User Type is Invalid!')
      ...
      .isIn(['trainee', 'admin']).withMessage('User Type should either be trainee or admin');
  ...
  workflow.on('createUserType', function createUserType(user, profile) {
    ...
    // Create User Type
    if (body.user_type === 'trainee') {
      TraineeDal.create({
        profile: profile._id,
      }, function callback1(err, trainee) {
        if (err) {
          return next(err);
        }
        ProfileDal.update({ _id: profile._id }, { $set: { trainee: trainee._id } }, function updateCb1(err, profile) {
